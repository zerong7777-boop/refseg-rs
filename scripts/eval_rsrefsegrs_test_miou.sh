#!/usr/bin/env bash
# Purpose: evaluate the RSRefSegRS test mIoU-best packaged checkpoint.
# Expected env: set REFSEG_RSREFSEGRS_DATA_ROOT; optionally source examples/env.sh first.
# Output: ${REFSEG_REPORT_JSON:-${REFSEG_OUTPUT_ROOT}/eval_rsrefsegrs_test_miou/report_t05.json}.
# Metric target: RSRefSegRS test mIoU-best checkpoint, evaluated at pred-threshold 0.5 by default.
set -euo pipefail
...
CHECKPOINT="${REFSEG_CHECKPOINT:-${CHECKPOINT_ROOT}/rsrefsegrs/rsrefsegrs_test_miou_best.state_dict.pth}"
THRESHOLD="${REFSEG_PRED_THRESHOLD:-0.5}"
MAX_SAMPLES="${REFSEG_MAX_SAMPLES:-0}"
REPORT_JSON="${REFSEG_REPORT_JSON:-${OUTPUT_ROOT}/eval_rsrefsegrs_test_miou/report_t05.json}"
