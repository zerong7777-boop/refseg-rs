#!/usr/bin/env bash
# Purpose: continue training from the refer_data_20250908 oIoU-best packaged checkpoint.
# Expected env: set REFSEG_REFER_DATA_ROOT; optionally source examples/env.sh first.
# Output: ${REFSEG_WORK_DIR:-${REFSEG_OUTPUT_ROOT}/resume_refer_oiou_<timestamp>}.
# Metric target: continue with best-metric oiou by default.
set -euo pipefail
...
WORK_DIR="${REFSEG_WORK_DIR:-${OUTPUT_ROOT}/resume_refer_oiou_${RUN_TAG}}"
